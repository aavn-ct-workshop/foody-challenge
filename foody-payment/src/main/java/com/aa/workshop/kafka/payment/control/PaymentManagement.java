...
public class PaymentManagement {
    ...
    private void handlePaymentFoodyOrder(FoodyOrder value) {
        ...
        if(balance >= value.getPrice()) {
            ...
            sendMessageToPaymentTopic(new FoodyWallet(value.getId(), value.getHostId(), value.getCreateDate(), ts.getTime(), true));
        } else {
            log.info("Produce message back to orders topic - {}", handledOrder);
            sendMessageToPaymentTopic(new FoodyWallet(value.getId(), value.getHostId(), value.getCreateDate(), ts.getTime(), false));
        }
    }

    private void sendMessageToPaymentTopic(FoodyWallet wallet) {
        //paymentsTemplate.send("payments", wallet);
    }
